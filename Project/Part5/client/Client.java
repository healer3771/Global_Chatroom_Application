...
import Project.Part5.common.Payload;
import Project.Part5.common.PayloadType;
import Project.Part5.common.RoomResultPayload;
...
public enum Client {
    ...
    private void processPayload(Payload p) {
        switch (p.getPayloadType()) {
            //CONNECT - when new client joins, sets name, gets name. will add the clientId/clientName pair to 
            //the HashTable if it doesn’t exist
            case CONNECT:
                ...
                System.out.println(String.format("*%s %s*",
                        ...
                        p.getMessage()));
                break;
            //DISCONNECT - when client disconnects remove the clientId/clientName pair from the HashTable if it exists
            case DISCONNECT:
                if (userList.containsKey(p.getClientId())) {
                ...
                System.out.println(String.format("*%s %s*",
                        ...
                        p.getMessage()));
                break;
            //SYNC_CLIENT - will add the clientId/clientName pair to the HashTable if it doesn’t exist 
            //(this is similar to connect but is “silent” and used to sync the user list to the person joining the room
            case SYNC_CLIENT:
                if (!userList.containsKey(p.getClientId())) {
                    userList.put(p.getClientId(), p.getClientName());
                }
                break;
            //MESSAGE - sends out message to client/clients and server 
            case MESSAGE:
                System.out.println(String.format("%s: %s",
                        getClientNameById(p.getClientId()),
                        p.getMessage()));
                break;
            //CLIENT_ID - Set ID when new Client joins
            case CLIENT_ID:
                ...
                break;
            //GET_ROOMS - gets to the rooms from the list of the room names when there are multiple rooms 
            case GET_ROOMS:
                ...
                break;
            //RESET_USER_LIST - is used to clear the user list to accept future clients
