    ...
    void processPayload(Payload p) {
        switch (p.getPayloadType()) {
            ...
            case DISCONNECT:
                ...
                break;
            //MESSAGE - sends out message to client and server
            case MESSAGE:
                if (currentRoom != null) {
                ...
                break;
            //GET_ROOMS - gets rooms when there are multiple rooms (triggers Room.getRoom() which get the data from server)
            // Rooms is that passess the data back to the client
            case GET_ROOMS:
                Room.getRooms(p.getMessage().trim(), this);
                break;
            //CREATE_ROOM - sent when client creates room to add new room to list (triggers Room.createRoom())
            case CREATE_ROOM:
                Room.createRoom(p.getMessage().trim(), this);
                break;
            //JOIN_ROOM - sent when client calls room to add client to room (tirggerd Room.joinRoom())
            case JOIN_ROOM:
                Room.joinRoom(p.getMessage().trim(), this);
                break;
            //READY - not used in chatroom but used in gameroom 
            case READY:
